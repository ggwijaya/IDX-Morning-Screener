...
3. **Analyze** — `analyze_one` computes indicators (RSI, Stochastic, Bollinger, OBV, MFI, MACD — pure pandas, no TA library) and four boolean oversold-rebound combo signals (`combo1`–`combo4`) per stock. The score counts only the combos selected in the sidebar multiselect (`COMBOS` constant, passed down as the `active` tuple): 2.5 per active combo, so the max score is 2.5 × number selected. The combo definitions are documented in the caption near the end of `main()`; if you change them, update that caption, `COMBOS`, and `BADGES` to match.
...
5. **Backtest marquee** — `backtest_yesterday` re-runs the screen on data up to the second-to-last bar (no look-ahead) with fixed `BT_*` constants (independent of sidebar settings), then marks each pick HIT/FAIL by whether the last bar's high reached close + `BT_HIT_PCT`%. `marquee_html` renders it as a CSS running text under the header.
